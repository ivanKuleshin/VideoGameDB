...
// Executes: SELECT * FROM VIDEOGAME WHERE ID = ?
// With: [101]

// Multiple parameters
// For INSERT with multiple columns:
...
```java
// When inserting (milliseconds to SQL Date)
new Date(videoGame.getReleaseDate())

// When retrieving (SQL Date to milliseconds)
// ObjectMapper handles this automatically via convertValue()
```
