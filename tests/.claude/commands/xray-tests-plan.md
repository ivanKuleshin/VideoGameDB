---
description: Read an XSP Story and plan one Xray manual Test issue per AC with detailed steps
argument-hint: <story-key e.g. XSP-115>
---
...
`/xray-tests-create` for implementation.

## Input

Story issue key: `$ARGUMENTS` (e.g. `XSP-115`).

If `$ARGUMENTS` is empty, ask the user for the story key and stop.

## Pre-flight: Read Context Files

Before planning anything, read these files:

- Source code: `app/src/main/java/com/ai/tester/resource/VideoGameResource.java`
- Security config: `app/src/main/java/com/ai/tester/config/SecurityConfig.java`
- Database schema: `app/src/main/resources/schema.sql`
- Data model: `app/src/main/java/com/ai/tester/model/VideoGame.java`

Then fetch the Story from Jira via `mcp__com_atlassian_atlassian-mcp-server__getJiraIssue` and parse all Acceptance Criteria.
...
7. End with: _"Plan complete. Review the test cases above, then run `/xray-tests-create` to implement."_
