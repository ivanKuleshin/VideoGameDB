---
description: Create a single Xray Test in Jira for an existing test method, mapped to an AC from a story
argument-hint: <STORY_KEY> <TEST_METHOD>
---
...
`$ARGUMENTS` should contain two whitespace-separated tokens:
...
If either is missing, ask the user to provide both and stop.

## Phase 1 — Gather Context

Run **in parallel**:

1. Fetch the story via `mcp__com_atlassian_atlassian-mcp-server__getJiraIssue` — extract all ACs from the description
2. Fetch the example test **XSP-131** via `mcp__xray__get_test_case` — use as format reference
3. Read the test source file containing `{TEST_METHOD}` — understand Given/When/Then logic

Use `mcp__com_atlassian_atlassian-mcp-server__getAccessibleAtlassianResources` to resolve `cloudId` if not already known.
...
Call `mcp__com_atlassian_atlassian-mcp-server__createJiraIssue`:
...
- `additional_fields`: `{ "labels": ["to_automate"], "priority": { "name": "Medium" } }`

## Phase 4 — Add Manual Steps

Add steps using `mcp__xray__add_test_step` in this fixed order — wait for each step to complete before adding the
next:
...
1. Call `mcp__com_atlassian_atlassian-mcp-server__createIssueLink` with type `Test`: new test issue **tests** `{STORY_KEY}`
...
Call `mcp__xray__get_test_case` for the created issue. Confirm:
...
> **{new_issue_key}** — [{summary}](https://ivankuleshin.atlassian.net/browse/{new_issue_key}) | covers `{STORY_KEY}` AC{N} | linked & annotated

## Rules

1. Never use `mcp__xray__create_test_case` — always use `createJiraIssue` with `issueTypeName: Test`
